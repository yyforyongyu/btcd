...
// calcP2SHScriptInfo updates ScriptInfo for spends whose pkScript class is
// pay-to-script-hash.  It handles both of the cases hidden behind P2SH:
...
// layer as well.
func calcP2SHScriptInfo(si *ScriptInfo, scriptVersion uint16,
	sigScript, pkScript []byte, witness wire.TxWitness, numInputs int,
	segwit bool) {
	...
	if segwit && IsWitnessProgram(redeemScript) {
		...
		if redeemClass == WitnessV0ScriptHashTy && len(witness) > 0 {
			...
			if wsInputs == -1 {
				si.ExpectedInputs = -1
			} else if si.ExpectedInputs != -1 {
				si.ExpectedInputs += wsInputs
			}
		}
		...
		si.NumInputs = len(witness) + numInputs
		return
	}
...
func CalcScriptInfo(sigScript, pkScript []byte, witness wire.TxWitness,
	...
	case si.PkScriptClass == ScriptHashTy && bip16:
		calcP2SHScriptInfo(
			si, scriptVersion, sigScript, pkScript, witness, numInputs, segwit,
		)
